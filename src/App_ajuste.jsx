...
import { ChevronDown, ChevronUp, PlusCircle, Edit3, Trash2, Copy, Search, Filter, XCircle, CheckCircle, CalendarDays, TrendingUp, PieChart as IconPieChart, Target, LogOut, Moon, Sun, Settings, Eye, EyeOff, Zap, AlertTriangle, MessageSquareText, ChevronLeft, ChevronRight, Sigma, Loader2 } from 'lucide-react'; 
import { 
    BarChart, Bar, LineChart, Line, PieChart as RechartsPieChart, Pie, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Cell, 
    AreaChart, Area // <-- Certifique-se de que AreaChart e Area estão aqui
} from 'recharts';
...
const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884D8', '#82Ca9D', '#FF7F50', '#DC143C', '#A020F0', '#FF69B4', '#FFD700', '#ADFF2F', '#7B68EE', '#DB7093', '#F08080'];
...
const DatePicker = ({ selectedDate, onChange, id }) => {
    ...
    // CLASSE CORRIGIDA: Usa classes dark mode que dependem do seletor global (html.dark)
...
const TransactionForm = ({ isOpen, onClose, onSave, editingTransaction, currentMonth }) => {
    ...
    const isAmountDisabled = isEditingInstallment && editingTransaction.isInstallmentPayment; // Permite edição do valor da parcela na mãe (isInstallmentOriginal)
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={editingTransaction ? "Editar Transação" : "Adicionar Transação"}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {isEditingInstallment && <p className="bg-yellow-500/20 text-yellow-400 p-3 rounded-lg text-sm flex items-center"><AlertTriangle size={18} className="mr-2"/> Não é possível alterar Tipo ou Data para esta transação.</p>}
...
const TransactionItem = ({ transaction, onEdit, onDelete, onSelect, isSelected, onTogglePaid }) => {
    ...
    return (
        <div className={`p-4 rounded-lg shadow-md transition-all ${isSelected ? 'ring-2 ring-cyan-500 bg-slate-700 dark:bg-gray-200' : 'bg-slate-700/50 hover:bg-slate-700 dark:bg-white dark:hover:bg-gray-100'}`}>
            <div className="flex items-start justify-between">
                <div className="flex items-center space-x-3">
                     <input type="checkbox" checked={isSelected} onChange={() => onSelect(id)} className="form-checkbox h-5 w-5 text-cyan-600 rounded cursor-pointer dark:text-cyan-700"/>
                    <div>
...
const InstallmentsReport = ({ userId }) => {
    ...
    return (
        <div className="bg-slate-800 dark:bg-white p-6 rounded-xl shadow-lg">
            <h3 className="text-2xl font-semibold text-violet-400 dark:text-violet-700 mb-6">Relatório de Compras Parceladas</h3>
            ...
            <div className="space-y-4">{installments.map(inst => {
                ...
                return (
                    <div key={inst.id} className="bg-slate-700/50 dark:bg-gray-100 p-4 rounded-lg">
                        <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleInstallmentDetails(inst.id)}>
                            <div><h4 className="text-lg font-medium text-slate-100 dark:text-slate-800">{inst.description}</h4><p className="text-sm text-slate-400 dark:text-gray-500">Total: R$ {(inst.totalAmount || 0).toFixed(2)} | Parcelas: {paidC}/{inst.numberOfInstallments || 0}</p><p className="text-xs text-slate-500 dark:text-gray-600">Compra: {fmtPDate}</p></div>
...
const Dashboard = ({ user, handleLogout, theme, toggleTheme }) => {
    ...
    const [targetMonthsForCopy, setTargetMonthsForCopy] = useState([]);
    ...
    const handleSaveTransaction = async (transactionData, transactionIdToEdit) => {
        ...
        try {
            ...
            if (!isNewTransaction && editingTransaction) { // Editando
                 ...
                 } else { // Transação normal ou pagamento de parcela
    ...
    const handleDeleteTransaction = async (transactionToDelete) => {
        if (!userId || !transactionToDelete || !transactionToDelete.id) return;
    ...
    const handleSelectTransaction = (id) => setSelectedTransactions(p => p.includes(id) ? p.filter(x => x !== id) : [...p, id]);
    ...
    const openCopyModal = () => {
        setCustomError('');
        if (selectedTransactions.length === 0) { setCustomError("Selecione uma transação para copiar."); return; }
        if (selectedTransactions.length > 1) {
            setSelectedTransactions([selectedTransactions[0]]); 
        }
        if (selectedTransactions.length === 1) {
            const transactionToCopy = combinedTransactions.find(t => t.id === selectedTransactions[0]);
            if (transactionToCopy && (transactionToCopy.isInstallmentOriginal || transactionToCopy.isInstallmentPayment)) {
                setCustomError("Não é possível copiar transações parceladas ou pagamentos de parcelas.");
                return;
            }
            setTargetMonthsForCopy([addMonths(currentMonth, 1)]); 
            setIsCopyModalOpen(true);
        }
    };

    const handleCopyTransactions = async () => { 
        if (selectedTransactions.length !== 1) { setCustomError("Selecione UMA transação para copiar."); setIsCopyModalOpen(false); return; }
        const transactionToCopyId = selectedTransactions[0];
        const originalTransaction = combinedTransactions.find(t => t.id === transactionToCopyId); 
        if (!originalTransaction) { setCustomError("Transação original não encontrada."); setIsCopyModalOpen(false); return; }
        if (originalTransaction.isInstallmentOriginal || originalTransaction.isInstallmentPayment) { 
            setCustomError("Não copie parcelamentos desta forma."); setIsCopyModalOpen(false); return; 
        }
        try {
            for (const targetMonth of targetMonthsForCopy) {
                const originalDate = originalTransaction.date.toDate ? originalTransaction.date.toDate() : new Date(originalTransaction.date);
                if (!isValid(originalDate)) {
                    setCustomError("Data original inválida."); setIsCopyModalOpen(false); return;
                }
                const newDate = new Date(originalDate);
                // Define o mês e ano da nova data, mantendo o dia da transação original.
                newDate.setMonth(getMonth(targetMonth)); 
                newDate.setFullYear(getYear(targetMonth));
                
                const newTransactionData = { 
                    ...originalTransaction, 
                    date: Timestamp.fromDate(newDate), 
                    // Limpando campos relacionados a parcelamento
                    isInstallment: false, isInstallmentOriginal: false, numberOfInstallments: undefined, 
                    valuePerInstallment: undefined, totalAmount: undefined,
            ...
            setIsCopyModalOpen(false); setSelectedTransactions([]);
        } catch (error) { console.error(error); setCustomError("Falha ao copiar transações."); setIsCopyModalOpen(false); }
    };
    const handleTargetMonthChange = (idx, date) => { 
        const n = [...targetMonthsForCopy]; 
        const validDate = isValid(date) ? date : (targetMonthsForCopy[idx] || new Date()); 
        n[idx] = validDate; 
        setTargetMonthsForCopy(n); 
    };
    const addTargetMonthField = () => setTargetMonthsForCopy([...targetMonthsForCopy, addMonths(targetMonthsForCopy[targetMonthsForCopy.length-1] || new Date(), 1)]);
    const removeTargetMonthField = (idx) => setTargetMonthsForCopy(targetMonthsForCopy.filter((_, i) => i !== idx));
    ...
    const categoryDataForPieChart = useMemo(() => {
        const expenseByCategory = filteredTransactions
            .filter(t => t.type === TRANSACTION_TYPES.EXPENSE && t.category)
            .reduce((acc, t) => {
    ...
    // CORREÇÃO FINAL DO GRÁFICO: Usando filteredTransactions
   const evolutionData = useMemo(() => {
    const daysInMonth = eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) });
    
    // 1. Crie um mapa de Receitas/Despesas por dia
    const dailyData = filteredTransactions.reduce((acc, t) => {
        const tD = t.date?.toDate();
        if (isValid(tD) && isSameMonth(tD, currentMonth)) {
            const dayKey = format(tD, 'yyyy-MM-dd'); 
            acc[dayKey] = acc[dayKey] || { Receitas: 0, Despesas: 0 };
            
            if (t.type === TRANSACTION_TYPES.INCOME) {
                acc[dayKey].Receitas += (t.amount || 0);
            } else {
                acc[dayKey].Despesas += (t.amount || 0);
            }
        }
        return acc;
    }, {});

    let runningBalance = 0; // Inicializa o saldo acumulado
    
    // 2. Mapeie os dias do mês, calculando o saldo acumulado
    return daysInMonth.map(day => {
        const dayKey = format(day, 'yyyy-MM-dd');
        const data = dailyData[dayKey] || { Receitas: 0, Despesas: 0 };
        
        const dailyBalance = data.Receitas - data.Despesas;
        runningBalance += dailyBalance; // Acumula o saldo
    ...
    return (
        <div className="min-h-screen bg-slate-900 dark:bg-gray-100 text-slate-100 dark:text-slate-900 p-4 md:p-8 transition-colors">
            <header className="mb-8 flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                <div className="flex items-center space-x-2 md:space-x-4">
                    <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-2 md:p-3 bg-slate-700 hover:bg-slate-600 dark:bg-gray-300 dark:hover:bg-gray-400 rounded-lg text-slate-100 dark:text-slate-900"><ChevronLeft size={24} /></button>
                    <h2 className="text-2xl md:text-3xl font-bold text-cyan-400 dark:text-cyan-700 capitalize tabular-nums min-w-[180px] text-center">{format(currentMonth, 'MMMM yy', { locale: ptBR })}</h2>
                    <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-2 md:p-3 bg-slate-700 hover:bg-slate-600 dark:bg-gray-300 dark:hover:bg-gray-400 rounded-lg text-slate-100 dark:text-slate-900"><ChevronRight size={24} /></button>
                </div>
                <div className="flex items-center space-x-4">
                    <button onClick={toggleTheme} className="p-2 bg-slate-700 hover:bg-slate-600 dark:bg-gray-300 dark:hover:bg-gray-400 rounded-lg text-slate-100 dark:text-slate-900">
                        {theme === 'dark' ? <Sun size={24} /> : <Moon size={24} />}
                    </button>
                    <p className="text-sm text-slate-400 dark:text-gray-600 hidden md:block">Usuário: {user.email?.split('@')[0]}</p>
                    <button onClick={handleLogout} className="bg-red-600 hover:bg-red-500 text-white font-semibold py-2 px-4 rounded-lg flex items-center"><LogOut size={20} className="mr-2" /> Sair</button>
                </div>
            </header>
            
            {customError && (<div className="mb-4 p-4 bg-red-800/50 text-red-300 border border-red-700 rounded-lg flex items-center justify-between dark:bg-red-100 dark:text-red-700 dark:border-red-400"><div className="flex items-center"><AlertTriangle size={20} className="mr-3 text-red-400 dark:text-red-700" /><span>{customError}</span></div><button onClick={() => setCustomError('')} className="text-red-300 hover:text-red-100 dark:text-red-700 dark:hover:text-red-900"><XCircle size={18} /></button></div>)}

            <div className="mb-8 p-6 bg-slate-800 dark:bg-white rounded-xl shadow-lg">
                <h3 className="text-2xl font-semibold text-sky-400 dark:text-sky-700 mb-4 flex items-center"><TrendingUp size={28} className="mr-3"/> Evolução Financeira Mensal</h3>
                <ResponsiveContainer width="100%" height={300}>                
                <LineChart data={evolutionData}>
                        <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? "#CBD5E0" : "#4A5568"} />
                        <XAxis dataKey="date" stroke={theme === 'dark' ? "#4A5568" : "#A0AEC0"} />
                        {/* Define 0 como o meio do gráfico */}
                        <YAxis stroke={theme === 'dark' ? "#4A5568" : "#A0AEC0"} domain={['auto', 'auto']} /> 
                        <Tooltip 
                            contentStyle={{ 
                                backgroundColor: theme === 'dark' ? '#E2E8F0' : '#2D3748', 
                                border: 'none', borderRadius: '0.5rem', 
                                color: theme === 'dark' ? '#1A202C' : '#A0AEC0' 
                            }} 
                            formatter={(value, name) => [`R$ ${value.toFixed(2)}`, name === 'SaldoAcumulado' ? 'Saldo Acumulado' : name]}
                        />
                        <Legend wrapperStyle={{ color: theme === 'dark' ? '#1A202C' : '#A0AEC0' }} />
                        
                        {/* Linha principal: Saldo Acumulado */}
                        <Line 
                            type="monotone" 
                            ...
                            stroke="#4299E1" 
                            strokeWidth={2} 
                            dot={false} // Mantém o visual limpo
                        />
                        
                        {/* Linha de referência no zero para ver o ponto de equilíbrio */}
                        <Line type="monotone" dataKey="zeroLine" stroke="#ccc" strokeDasharray="3 3" dot={false} activeDot={false} hide={true}/>
                    </LineChart>
                </ResponsiveContainer>
                {/* FIM DO NOVO BLOCO DO GRÁFICO */}
                
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-slate-800 dark:bg-white p-6 rounded-xl shadow-lg text-center"><h4 className="text-lg font-medium text-green-400 dark:text-green-700 mb-2">Receitas</h4><p className="text-3xl font-bold">R$ {monthlySummary.income.toFixed(2)}</p></div>
                <div className="bg-slate-800 dark:bg-white p-6 rounded-xl shadow-lg text-center"><h4 className="text-lg font-medium text-red-400 dark:text-red-700 mb-2">Despesas</h4><p className="text-3xl font-bold">R$ {monthlySummary.expense.toFixed(2)}</p></div>
                <div className="bg-slate-800 dark:bg-white p-6 rounded-xl shadow-lg text-center"><h4 className="text-lg font-medium text-sky-400 dark:text-sky-700 mb-2">Saldo</h4><p className="text-3xl font-bold">R$ {monthlySummary.balance.toFixed(2)}</p></div>
                <div className="bg-slate-800 dark:bg-white p-6 rounded-xl shadow-lg flex flex-col justify-between">
                    <h4 className="text-lg font-medium text-purple-400 dark:text-purple-700 mb-2 flex items-center"><MessageSquareText size={22} className="mr-2"/> Dica IA</h4>
                    {isLoadingInsight && <p className="text-sm text-slate-400 dark:text-gray-500 animate-pulse flex items-center"><Loader2 size={16} className="mr-2 animate-spin"/> Gerando...</p>}
            ...
            <div className="mb-8 p-6 bg-slate-800 dark:bg-white rounded-xl shadow-lg">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <h3 className="text-2xl font-semibold text-cyan-400 dark:text-cyan-700">Transações do Mês</h3>
                     <div className="flex items-center space-x-2">
                        <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-2 bg-slate-700 hover:bg-slate-600 dark:bg-gray-300 dark:hover:bg-gray-400 rounded-lg text-slate-100 dark:text-slate-900"><ChevronLeft size={20} /></button>
                        <span className="text-slate-300 dark:text-slate-800 font-medium tabular-nums min-w-[150px] text-center">{format(currentMonth, 'MMMM yy', { locale: ptBR })}</span>
                        <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-2 bg-slate-700 hover:bg-slate-600 dark:bg-gray-300 dark:hover:bg-gray-400 rounded-lg text-slate-100 dark:text-slate-900"><ChevronRight size={20} /></button>
                    </div>
                </div>
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
                    <div className="relative w-full sm:w-auto">
                        <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="p-3 pl-10 bg-slate-700 rounded-lg text-slate-100 w-full dark:bg-gray-200 dark:text-slate-900"/>
                        <Search size={20} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 dark:text-gray-500"/>
                    </div>
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        <select value={transactionTypeFilter} onChange={(e) => setTransactionTypeFilter(e.target.value)} className="p-3 bg-slate-700 rounded-lg text-slate-100 flex-grow sm:flex-grow-0 dark:bg-gray-200 dark:text-slate-900">
                            <option value="all">Todas</option><option value={TRANSACTION_TYPES.INCOME}>Receitas</option><option value={TRANSACTION_TYPES.EXPENSE}>Despesas</option>
                        </select>
                        <button onClick={() => { setEditingTransaction(null); setIsModalOpen(true); }} className="bg-cyan-600 hover:bg-cyan-500 dark:bg-cyan-700 dark:hover:bg-cyan-600 text-white font-semibold py-3 px-5 rounded-lg flex items-center justify-center flex-grow sm:flex-grow-0"><PlusCircle size={20} className="mr-2" /> Nova</button>
                    </div>
                </div>

                {/* Totalizador Dinâmico */}
                <div className="mb-4 p-4 bg-slate-700/50 dark:bg-gray-200 rounded-lg flex items-center justify-between">
                    <div className="flex items-center text-slate-300 dark:text-slate-700">
                        <Sigma size={20} className="mr-2 text-sky-400 dark:text-sky-700" />
                        <span className="font-semibold">Saldo das Transações Visíveis:</span>
                    </div>
                    <span className={`text-xl font-bold ${totalFilteredTransactionsAmount >= 0 ? 'text-green-400 dark:text-green-700' : 'text-red-400 dark:text-red-700'}`}>
                        R$ {totalFilteredTransactionsAmount.toFixed(2)}
                    </span>
                </div>

                <div className="mb-4 flex items-center space-x-3">
                    <input type="checkbox" id="selectAll-trans" checked={filteredTransactions.length > 0 && selectedTransactions.length === filteredTransactions.length} onChange={handleSelectAllTransactions} disabled={filteredTransactions.length === 0} className="form-checkbox h-5 w-5 text-cyan-600 dark:text-cyan-700 rounded disabled:opacity-50"/>
                    <label htmlFor="selectAll-trans" className={`text-sm text-slate-300 dark:text-slate-700 ${filteredTransactions.length === 0 ? 'opacity-50' : 'cursor-pointer'}`}>Selecionar Todas Visíveis</label>
                    {selectedTransactions.length > 0 && (<button onClick={openCopyModal} className="ml-4 bg-sky-600 hover:bg-sky-500 dark:bg-sky-700 dark:hover:bg-sky-600 text-white text-sm py-2 px-3 rounded-lg flex items-center"><Copy size={16} className="mr-2" /> Copiar</button>)}
                </div>

                {globalLoading ? (<p className="text-center text-slate-400 dark:text-gray-500 py-8 flex items-center justify-center"><Loader2 size={24} className="mr-2 animate-spin"/> Carregando transações...</p>) : 
                filteredTransactions.length === 0 ? (<p className="text-center text-slate-400 dark:text-gray-500 py-8">Nenhuma transação encontrada para este mês ou filtro.</p>) : 
                (<div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">{filteredTransactions.map(transaction => (<TransactionItem key={transaction.id} transaction={transaction} onEdit={handleEditTransaction} onDelete={handleDeleteTransaction} onSelect={handleSelectTransaction} isSelected={selectedTransactions.includes(transaction.id)} onTogglePaid={handleToggleInstallmentPaymentPaid}/>))}</div>)}
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                <div className="bg-slate-800 dark:bg-white p-6 rounded-xl shadow-lg">
                    <h3 className="text-2xl font-semibold text-amber-400 dark:text-amber-700 mb-4 flex items-center"><IconPieChart size={28} className="mr-3"/> Despesas/Categoria</h3>
                    {categoryDataForPieChart.length > 0 ? (
                    <ResponsiveContainer width="100%" height={300}>
                        <RechartsPieChart>
                            <Pie data={categoryDataForPieChart} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} labelLine={false} label={({ name, percent }) => `${name} (${(percent * 100).toFixed(0)}%)`}>
                                {categoryDataForPieChart.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.fill} />))}
                            </Pie>
                            <Tooltip formatter={(value) => `R$ ${value.toFixed(2)}`} contentStyle={{ backgroundColor: theme === 'dark' ? '#E2E8F0' : '#2D3748', border: 'none', borderRadius: '0.5rem', color: theme === 'dark' ? '#1A202C' : '#A0AEC0' }} />
                            <Legend wrapperStyle={{ color: theme === 'dark' ? '#1A202C' : '#A0AEC0' }}/>
                        </RechartsPieChart>
                    </ResponsiveContainer>
                    ) : (<p className="text-slate-400 dark:text-gray-500 text-center py-10">Sem dados de despesas para o gráfico.</p>)}
                </div>
                <InstallmentsReport userId={userId} />
            </div>
            ...
            <Modal isOpen={isCopyModalOpen} onClose={() => setIsCopyModalOpen(false)} title="Copiar Transação">{selectedTransactions.length === 1 && combinedTransactions.find(t => t.id === selectedTransactions[0]) && (<div className="space-y-4"><p className="text-slate-300 dark:text-slate-700">Copiar: <span className="font-semibold text-cyan-400 dark:text-cyan-700">{combinedTransactions.find(t => t.id === selectedTransactions[0])?.description}</span></p><p className="text-slate-300 dark:text-slate-700">Para os meses:</p>{targetMonthsForCopy.map((month, index) => (<div key={index} className="flex items-center space-x-2"><DatePicker selectedDate={month} onChange={(date) => handleTargetMonthChange(index, date)} id={`copy-month-${index}`} />{targetMonthsForCopy.length > 1 && (<button onClick={() => removeTargetMonthField(index)} className="p-2 text-red-500"><Trash2 size={18}/></button>)}</div>))}<button onClick={addTargetMonthField} className="text-sm text-cyan-400 dark:text-cyan-700 flex items-center"><PlusCircle size={16} className="mr-1"/> Add Mês</button><div className="flex justify-end space-x-3 pt-4"><button onClick={() => setIsCopyModalOpen(false)} className="px-6 py-3 bg-slate-600 dark:bg-gray-300 dark:text-slate-900 rounded-lg">Cancelar</button><button onClick={handleCopyTransactions} className="px-6 py-3 bg-sky-600 dark:bg-sky-700 text-white rounded-lg">Copiar</button></div></div>)}</Modal>
            <footer className="mt-12 text-center text-sm text-slate-500 dark:text-gray-400"><p>Financial Control App &copy; {new Date().getFullYear()}</p><p className="text-xs">App ID: {appId}</p></footer>
        </div>
    );
};
...
export default function App() {
    ...
    useEffect(() => {
     ...
     if (theme === 'dark') {
        ...
        document.documentElement.classList.remove('light'); // Adicionado para clareza
    } else {
        document.documentElement.classList.remove('dark');
        document.documentElement.classList.add('light'); // Adicionado para clareza
    }
    }, [theme]);
    ...
    return <Dashboard user={user} handleLogout={handleLogout} theme={theme} toggleTheme={toggleTheme} />;
}